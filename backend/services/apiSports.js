...
// mucho ojo con la referencia circular esto no se puede hacer
const { getSportUrl } = require('../utils/urlHelper');

const apiSportsKey = process.env.API_SPORTS_KEY;


function quota(response, nombreFuncion) {
    console.log('Llamada de: ', nombreFuncion);
    console.log('Solicitudes Usadas: ', response.headers['x-ratelimit-requests-limit']
        - response.headers['x-ratelimit-requests-remaining']);
    console.log('Solicitudes restantes: ', response.headers['x-ratelimit-requests-remaining']);
    console.log('*************************');
}


async function getAllMatches(sportKey) {
    console.log('getAllMatches');
    console.log('sportKey: ',sportKey);
    const theSportUrl = getSportUrl(sportKey.toLowerCase());
    console.log('theSportUrl:', theSportUrl);

    const response = await axios.get(`${theSportUrl}/fixtures`, {
        headers: {
    ...
    quota(response, "getAllMatches");
    return response.data;
}

async function getFixture(sportKey, fixtureId) {
    ...
    quota(response, "getFixture");
    return response.data;
}

async function getLiveOdds(sportKey) {
    console.log('getLiveOdds');
    const theSportUrl = getSportUrl(sportKey.toLowerCase());
    console.log('theSportUrl:', theSportUrl);
    const response = await axios.get(`${theSportUrl}/odds/live`, {
        headers: {
            'x-apisports-key': apiSportsKey
        }
    });
    quota(response, "getLiveOdds");
    console.log(response.data)
    return response.data;
    /*    JSON estático para no consumir solicitudes
        const response = {
            "get": "odds/live",
            "parameters": {
                "fixture": "721238"
...
async function getFixtureOdds(sportKey, fixtureId) {
    console.log('getFixtureOdds');
    const theSportUrl = getSportUrl(sportKey.toLowerCase());
    console.log('theSportUrl:', theSportUrl);
...
async function getUpcomingOdds(sportKey, date) {
    console.log('getFixtureOdds');
    const theSportUrl = getSportUrl(sportKey.toLowerCase());
    console.log('theSportUrl:', theSportUrl);
    console.log('date', date);
    const response = await axios.get(`${theSportUrl}/odds`, {
        headers: {
            'x-apisports-key': apiSportsKey
        },
        params: {
    ...
    quota(response, "getFutureOdds");
    console.log(response.data)
    return response.data;
}

async function getTeamName(sportKey, teamId) {
    const theSportUrl = getSportUrl(sportKey.toLowerCase());
    const response = await axios.get(`${theSportUrl}/teams`, {
        headers: {
            'x-apisports-key': apiSportsKey
        },
        params: {
