...
/**
 * Props:
 * - fill        : color del área de relleno (grupo .basket_fill)
 * - fillOpacity : opacidad del relleno
 * - line        : color de líneas (grupo .basket_line)
 * - lineWidth   : grosor de líneas (px)
 * - extra       : color de líneas extra (grupo .basket_extra-line)
 * - className   : clases extra para el wrapper
 *
 * Métodos expuestos por ref:
 * - getSVG()       -> devuelve el <svg> para manipularlo directamente
 * - setColors(obj) -> cambia vars CSS en caliente: {fill, line, extra, lineWidth, fillOpacity}
 * - query(sel)     -> svg.querySelectorAll(sel)
 */
const Basket = forwardRef(function Basket(
  {
    fill = "#f5f5f5ff",
    fillOpacity = 0.30,
    line = "#F59E0B",
  ...
  // ⬇️ Pega aquí tu <svg …>…</svg> completo (tal cual lo tienes en tu archivo actual)
  //    No hace falta tocar nada del contenido.
  const SVG_RAW = useMemo(
  ...
  // Aplicamos “reactificación”: forzamos tamaño, visibilidad y colores controlados por CSS vars
  useEffect(() => {
    const host = rootRef.current;
    if (!host) return;

    const svg = host.querySelector("svg#basket");
    if (!svg) return;

    // Asegura que ocupa la carta
    svg.setAttribute("width", "100%");
    svg.setAttribute("height", "100%");
    if (!svg.getAttribute("preserveAspectRatio")) {
      svg.setAttribute("preserveAspectRatio", "xMidYMid meet");
    }
    svg.style.opacity = "1"; // algunos assets vienen con opacity="0"

    // Grupos principales
    const gFill  = svg.querySelector(".basket_fill");
    const gLine  = svg.querySelector(".basket_line");
    const gExtra = svg.querySelector(".basket_extra-line");

    // 1) Normalizamos estilos inline para que NO bloqueen nuestros colores
    //    (quitamos fill/stroke en hijos si lo traen “hardcodeado”)
    const stripAttrs = (root, attrs = ["fill", "stroke"]) => {
      if (!root) return;
      root.querySelectorAll("*").forEach((el) => {
        ...
        if (style) {
          let s = style;
          attrs.forEach((a) => {
            // elimina "fill:#fff" / "stroke:#000" del style inline
            s = s.replace(new RegExp(`${a}\\s*:[^;]*;?`, "gi"), "");
          });
    ...
    stripAttrs(gFill,  ["fill"]);
    stripAttrs(gLine,  ["stroke", "fill"]);
    stripAttrs(gExtra, ["stroke", "fill"]);

    // 2) Reaplicamos estilos base para que hereden de variables CSS
    if (gFill) {
      gFill.style.fill = "var(--basket-fill)";
      gFill.style.opacity = "var(--basket-fill-opacity)";
      // asegura que todo herede
      gFill.querySelectorAll("*").forEach((el) => (el.style.fill = "inherit"));
    }
    if (gLine) {
      gLine.style.fill = "none";
      gLine.style.stroke = "var(--basket-line)";
      gLine.style.strokeWidth = "var(--basket-line-width)";
      gLine.querySelectorAll("*").forEach((el) => {
        el.style.fill = "none";
        el.style.stroke = "inherit";
        el.style.strokeWidth = "inherit";
      });
    }
    if (gExtra) {
      gExtra.style.fill = "none";
      gExtra.style.stroke = "var(--basket-extra)";
      gExtra.style.strokeWidth = "var(--basket-line-width)";
      gExtra.querySelectorAll("*").forEach((el) => {
        el.style.fill = "none";
        el.style.stroke = "inherit";
        el.style.strokeWidth = "inherit";
      });
  ...
  // Expone helpers para manipular el SVG desde fuera (animaciones, recoloraciones, etc.)
  useImperativeHandle(ref, () => ({
    getSVG: () => rootRef.current?.querySelector("svg#basket") || null,
    setColors: ({
      fill: f,
      fillOpacity: fo,
      line: l,
      ...
      if (f  != null) rootRef.current.style.setProperty("--basket-fill", f);
      if (fo != null) rootRef.current.style.setProperty("--basket-fill-opacity", String(fo));
      if (l  != null) rootRef.current.style.setProperty("--basket-line", l);
      if (lw != null) rootRef.current.style.setProperty("--basket-line-width", String(lw));
      if (e  != null) rootRef.current.style.setProperty("--basket-extra", e);
    },
    query: (sel) =>
      Array.from(
        rootRef.current?.querySelector("svg#basket")?.querySelectorAll(sel) || []
      ),
  }));
  ...
  return (
    <div
      ref={rootRef}
      className={`basket-react ${className}`}
      // Variables CSS controladas por props
      style={{
        "--basket-fill": fill,
        "--basket-fill-opacity": String(fillOpacity),
        "--basket-line": line,
        "--basket-line-width": String(lineWidth),
        "--basket-extra": extra,
      }}
