...
import useSoccer1Anim from "@/components/anim/useSoccer1Anim";

const Soccer1 = forwardRef(function Soccer1({ className = "" }, ref) {
  const rootRef = useRef(null);
  const { play, pause, reset } = useSoccer1Anim(rootRef);

  useImperativeHandle(ref, () => ({ play, pause, reset }), []);

  // 🔧 Forzar visibilidad y tamaño del SVG al montar
  useEffect(() => {
    const svg = rootRef.current?.querySelector("svg#soccer1");
    if (!svg) return;
    svg.removeAttribute("opacity");      // venía con opacity="0"
    svg.style.opacity = "1";
  ...
  return (
    <div ref={rootRef} className={`soccer1-react ${className}`} style={{ width: "100%", height: "100%" }}>
      <div
        dangerouslySetInnerHTML={{
          __html: `<svg xmlns="http://www.w3.org/2000/svg" opacity="0" xmlns:xlink="http://www.w3.org/1999/xlink" id="soccer1" x="0px" y="0px" enable-background="new 0 0 2948.4 2312.3" version="1.1" viewBox="0 0 2948.4 2312.3" xml:space="preserve">
        <title>
            soccer-1
        </title>
        <g class="soccer1ball">
